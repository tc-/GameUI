...
namespace Ui
{
	...
	class TileView : public Widget
	{
		public:
			...
			virtual ImageList& tileImages(  ) {
				return pTileImages;
			}
			virtual ImageList& objectImages(  ) {
				return pObjectImages;
			}
			virtual ImageList& spriteImages(  ) {
				return pSpriteImages;
			}

			virtual bool showGrid(  ) const {
				return pShowGrid;
			}
			virtual void showGrid( const bool enable ) {
				pShowGrid = enable;
				updated();
			}
			virtual Color gridColor(  ) const {
				return pGridColor;
			}
			virtual void gridColor( const Color c ) {
				pGridColor = c;
				if ( pShowGrid ) updated();
			}

			virtual List<TileSprite>& sprites(  ) {
				return pSprites;
			}

			virtual bool drawTiles(  ) const {
				return pDrawTiles;
			}
			virtual bool drawObjects(  ) const {
				return pDrawObjects;
			}
			virtual bool drawSprites(  ) const {
				return pDrawSprites;
			}
			virtual bool drawOverlays(  ) const {
				return pDrawOverlays;
			}

			virtual void setDrawTiles( const bool enable ) {
				pDrawTiles = enable;
			}
			virtual void setDrawObjects( const bool enable ) {
				pDrawObjects = enable;
			}
			virtual void setDrawSprites( const bool enable ) {
				pDrawSprites = enable;
			}
			virtual void setDrawOverlays( const bool enable ) {
				pDrawOverlays = enable;
			}
			...
			virtual void setTileWidth( const int tw ) {
				pTileWidth = tw;
			}
			virtual void setTileHeight( const int th ) {
				pTileHeight = th;
			}

			virtual Tile& tiles( const unsigned int x, const unsigned int y ) {
				assert( ( y*pNumX+x ) < pNumX*pNumY );
				return pTiles[y*pNumX+x];
			}

			virtual void tileUpdated( const unsigned int x, const unsigned int y ) {
				updated();
			}

			virtual unsigned int numTilesX(  ) const {
				return pNumX;
			}
			virtual unsigned int numTilesY(  ) const {
				return pNumY;
			}
			...
			virtual void tileUpdated( const int x, const int y ) {
				updated();
			}
