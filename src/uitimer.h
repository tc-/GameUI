...
namespace Ui
{
	...
	class TimerControl
	{
		public:
			...
			/**
			 * The number of milliseconds left until the timer should "tick".
			 *
			 * This function should normally not be used. Insted use the timeLeft() function.
			 ...
			 * @see removeTimer() Timer::createTimerControl().
			 */
			 ...
			 * @see assignTimer() Timer::destroyTimerControl().
			 */
			 ...
			 * @see setCreateTimerControl() Timer::TimerControlFactory.
			 */
			 ...
			 * @see setDestroyTimerControl() Timer::TimerControlDestroyer.
			 */
	 ...
	 *
	 * @see timeout() tick() onTimer enabled() TimerControl.
	 */
	class Timer : public has_slots<>
	{
		public:
			 ...
			 * @see tick() timeout() TimerControl::getTimeLeft().
			 */
			 ...
			 * @see timeout().
			 */
			virtual void setTimeout( const int& t ) {
				pTimeout = t;
			}
			/**
			 * Reset the timer.
			...
			/**
			 * True if the timer is enabled and false if the timer is disabled.
			 *
			...
			virtual void setEnabled( const bool enable = true ) {
				if ( pEnabled != enable ) {
					pEnabled = enable;
					onEnabled( *this, pEnabled );
				}
			}

			/**
			 * Pointer to the TimerControl responsible for handeling this timer.
			 *
			 * @return a pointer to the TimerControl responsible for handeling this timer or NULL if none assigned.
			 * @see TimerControl setTimerControl().
			 */
			virtual TimerControl* timerControl() {
				return pTC;
			}
			 ...
			 *
			 * If you want to associate something with this timer, use this property to do that. The tcTag() property is ONLY for use by the TimerControl classes.
			...
			/**
			 * Pointer used by the TimerControl returned by timerControl().
			 *
			 * @note This property should ONLY be used by the TimerControl returned by timerControl(), the data stored in this pointer can vary from one TimerControl class to another.
			 * @return a pointer used by the TimerControl returned by timerControl().
			 * @see setTCTag() TimerControl.
			 */
			virtual void* tcTag(  ) {
				return pTCTag;
			}
			...
			/**
			 * Set a pointer used by the TimerControl returned by timerControl().
			 *
			 * @note This property should ONLY be used by the TimerControl returned by timerControl(), the data stored in this pointer can vary from one TimerControl class to another.
			 * @param p a pointer used by the TimerControl returned by timerControl().
			 * @see tcTag() TimerControl.
			 */
			virtual void setTCTag( void* p ) {
				pTCTag = p;
			}
